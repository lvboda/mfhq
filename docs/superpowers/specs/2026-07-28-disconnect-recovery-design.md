...
**A 类用两个信号检测，不只看掉线提示框。** 提示框是瞬态的——它可能自行消失，或客户端已回到登录界面。若只在 300 秒的采样点上找这一张图，存活时间短于采样周期的提示框基本不会被观察到，而此时进程仍然存活，`ensure_game_ready()` 会直接返回，脚本便在登录界面上无限空转且永不恢复。因此同时检测登录按钮 `const.denglu`：它只在登录界面出现，正常游戏过程中不可见，作为"已掉回登录界面"的判据比提示框稳定。
